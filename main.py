...
def verifica_dominacao_romana_total(grafo, atribuicoes):
    ...
    for v in range(n):
        ...
        if atribuicoes[v] in [1, 2] and not any(atribuicoes[u] >= 1 for u in grafo[v]):
            print("Existe um vértice com guarnição [1, 2] que não está ligado a pelo menos um vértice com duas guarnições")
            return False
...
"""
def atribuicao_parcial(grafo, atribuicoes):
    n = len(grafo)
    for v in range(n):
        if atribuicoes[v] == -1:
            continue
...
"""
    Esta heurística encontrou um ótimo local (8) e sem heurística encontrou (6)
    Ela super estima o mínimo
"""
def heuristica_limite_inferior(atribuicoes):
    ...
    estimativa_minima = vertices_nao_atribuídos  # estimativa de 1 por vértice ainda não atribuído
    return soma_atribuida + estimativa_minima
...
def branch_bound(grafo, atribuicoes, vertice, melhor_solucao, melhor_peso, logfile, contador_vizinhos):
    ...
    for valor_guarnicao in [0, 1, 2]:
        ...
        # Função anterior
        #if not atribuicao_parcial(grafo, atribuicoes):
        if not valido_parcial_v2(contador_vizinhos, vertice, valor_guarnicao):
...
"""
    ...
    """
def valido_parcial_v2(contador_vizinhos, v, valor):
    if valor == 0:
...
grafo = matriz_adjacencia(arquivo)
calcular(grafo)
